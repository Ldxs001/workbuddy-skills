...
**错误做法**：把 `holiday_intervals.json` 和 `schedule_events.json` 的完整字段说明全部写在 SKILL.md 里，导致文件超过 200 行，违反 R-17 规则。
...
**错误做法**：手动用编辑器打开 `data/schedule_events.json` 修改日程，改错后无法恢复，也没有利用系统自带的 .bat 容灾备份机制。
