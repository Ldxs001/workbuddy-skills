...
| 关键词存在 | 含 `渐进式加载`、`progressive`、`SKILL.md 为入口` 等关键词之一 | 在显眼章节添加渐进式加载说明 |
| 说明完整 | 说明 `SKILL.md` 为入口、拆分到 `references/*.md`、按需加载 | 补充完整说明 |
...
（用本技能创建/更新/改造的技能，均会在 `## 核心能力` 或 `## 工作流程` 章节显式说明渐进式加载。）
