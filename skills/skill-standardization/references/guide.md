...
#### R-20: 写作规范（术语一致/无模糊表述/中英文混排/功能贴合）（WARN）

检查正文写作规范：术语一致性、禁止模糊表述、中英文混排空格、功能贴合性。
...
| 功能列表存在 | `## 核心能力` 章节包含功能列表（表格格式） | 添加功能列表（\| # \| 功能 \| 说明 \|）|
| 参数说明 | `scripts/*.py` 含 `argparse`/`sys.argv` 时，`SKILL.md` 应有 `## 参数` 或 `## 使用方法` 章节 | 添加参数说明章节 |

**v2.24.2 新增：功能贴合性检查**：
- 检查 `## 核心能力` 章节是否包含功能列表（表格格式）
- 检查如果 `scripts/*.py` 有命令行参数，是否在 `SKILL.md` 里说明
