...
def find_engine(engine: str) -> str:
    """查找 LaTeX 引擎路径（与 validate.py 的 find_engine 一致）"""
    candidates = [
        rf"C:\Program Files\MiKTeX\miktex\bin\x64\{engine}.exe",
        rf"/c/Program Files/MiKTeX/miktex/bin/x64/{engine}",
        rf"/c/texlive/2024/bin/windows/{engine}",
        f"/usr/bin/{engine}",
        f"/usr/local/bin/{engine}",
        engine  # 系统 PATH 中查找
    ]
    for p in candidates:
        if os.path.exists(p):
...
def build_document(manifest: Dict, base_dir: str) -> str:
...
def main():
    ...
    document = build_document(manifest, base_dir)
