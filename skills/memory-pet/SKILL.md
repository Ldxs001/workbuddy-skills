...
license: MIT
description: 宠物记忆压缩技能 - 通过文本块宠物交互触发上下文压缩与记忆保存。每只宠物独立记忆文件，亲密度衰减与逃跑机制，Python 全程管理。
...
| 用户选择 | CLI 命令 | 说明 |
|---------|---------|------|
| 干饭 | `interact eat <delta> --food X --taste Y` | 记忆保存 + 上下文压缩 |
| 散步 | `interact walk <delta> --scene X` | 50% 遇新宠，随机种类 |
| 贴贴 | `interact cuddle <delta> --scene X` | 亲密度大幅变化 |
| 回忆 | `recall <pet_id>` | 获取记忆列表 |
