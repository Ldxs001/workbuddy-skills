...
模型文件存储在：
  ...
  └── qwythos-9b-q4/               ← Qwythos GGUF ~5.5GB（可选，需GPU）
...
| `references/execution_standards.md` | 参考文档 | 字数管理/文体规范/别名系统/实体追踪/check链说明/结尾收束 | 无 |
...
| **子结构原子写入** | 逐子结构写入正文，自动校验格式/字数/署名/行号，写后自动提取实体关系与行为摘要 | write-sub 管道（atomic_writer + state_manager + entity_extractor），字数按篇幅目标校验，上限+15% 浮动窗口 |
| **因果链验证** | 章级和子结构级双模式因果递进验证 | 仅检查关键词重叠和因果递进，无法验证剧情合理性 |
| **章节完结检查** | 章内连通性+跨章承诺链+风格校验+逻辑检查+**语义检查(BERT，可选)**+**推理审核(Qwythos，可选，需GPU)** — 共6步 | HARD问题阻断并生成_fixes.json，需人工修复；语义+推理无模型时自动跳过 |
| **实体关系追踪** | 写后自动提取角色/物品/地点/组织/数据五类实体及其关系，context_loader注入，finalize-chapter校验状态一致性 | 提取由规则引擎驱动，复杂关系可能漏提 |
...
- **网络**：核心流程完全离线。BERT 语义检查需首次下载模型 33MB（可选）；Qwythos 推理审核需下载 ~5.5GB 模型（可选，需 GPU）
...
10. **加载命题指令** → 输入 chapter+sub_key → 输出 context_loader输出 — 子结构规划+字数约束（篇幅字数范围+15%上浮）+已出场关键人物（含别名列表）+实体关系网（角色/物品/地点/组织/数据五类+关联关系）+上一章行为轨迹+情绪混合（强度分级+混合解读）+人格约束(MBTI+原型)+文风约束(6字段)+**硬性别名声明（缺失则阻断）+串行阻断检查（上一子结构pending则HOOK-BLOCK）**
11. **LLM写作+管道写入(write-sub)** → 输入 命题指令 → 输出 S##.txt — atomic_writer代码级校验格式/字数/署名 + state_manager更新进度（字数对标篇幅目标，OK/WARN/INFO 三级输出）+ entity_extractor自动提取实体关系（非阻断）
12. **重复10-11** → 输入 下一子结构 → 输出 全部子结构完成 — 直到该章全部子结构写完
13. **完结一章(finalize-chapter)** → 输入 章内容 → 输出 六合一检查报告 — 章内连通性→跨章→风格→逻辑（含实体状态+关系链）→**语义检查(BERT)**→**推理审核(Qwythos，可选)**+ 行为摘要提取 — 通过后自动提取角色行为轨迹写入state
