...
| **推理审核(Qwythos)** | **finalize-chapter 时（第6步）** | **HARD+SOFT（可选，需 GPU+模型）** | novel_reasoning_check.py — 5 项推理审核（因果合理性/人物行为一致/情绪弧自然度/对话匹配度/论证可靠性），按结果输出 HARD 或 SOFT | `scripts/novel_reasoning_check.py` |
