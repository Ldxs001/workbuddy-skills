...
| 子结构先行规划 | 每章写作前 | **代码级硬约束+门禁** | workflow_engine.py plan-chapter 批量注册后自动 pass plan_chapter:L## 门禁 | `scripts/novel_workflow_engine.py` |
...
| 逻辑一致性检查 | 每章完成后 | 自动式+阶段门禁 | novel_logic_check.py（人物行为/时间线/内容匹配度） | `scripts/novel_logic_check.py` |
| 一键完结章节 | 全部检查后 | 编排式+门禁 | workflow_engine.py finalize-chapter → cont+style+logic+pass chapter_finalized:L## | `scripts/novel_workflow_engine.py` |
...
|------|---------|-----------|-------------|---------|
| `outline_causality` | 章概述因果链 | causality_check.py | set-phase → writing | LLM 无法开始写作 |
| `plan_chapter:L##` | 子结构注册状态 | workflow_engine.py | — | 间接阻断（无子结构会触发 context_loader 报错） |
| `sub_causality:L##` | 子结构因果链 | causality_check.py | — | 间接阻断（写作流程无此门禁会触发写作混乱） |
| `chapter_finalized:L##` | 章完结报告 | workflow_engine.py | set-phase → stage3_ready | LLM 无法结束全书 |
| `fidelity` | 大纲忠实度 | fidelity.py | set-phase → complete | LLM 无法完结项目 |
