...
| 层级 | 上限 | 说明 |
|------|------|------|
| 每个子结构 | 230 行（自然段落结束） | 以叙事单位结束，不强行撑到上限 |
| 每章（汇总） | 不限（多个子结构累计） | 由 3-5 个子结构的自然累加决定 |
| 全文 | 8-15 章（推荐） | 触发时由大纲章节数决定，不固定 |

### 按篇幅字数参考

| 篇幅 | 总字数 | 每章字数 | 每子结构字数 | 行数参考（每子结构） |
|------|--------|---------|-------------|--------------------|
| 短篇 | 1.5-3 万 | 4,000-6,000 | 1,000-1,500 | 25-40 行 |
| 中篇 | 4-6 万 | 6,000-8,000 | 1,500-2,000 | 40-55 行 |
| 长篇 | 8 万+ | 8,000-15,000 | 2,000-4,000 | 55-100 行 |

- 每个子结构字数 = 章字数 / 该章子结构数（3-5 个）
- 实际字数以叙事单位自然结束为准，不强行撑到目标
- **代码级校验**：write-sub 写入后自动对比字数是否达标，低于篇幅下限打印 `[WARN]`
- 所有篇幅均在 230 行/子结构的安全上限内，字数瓶颈来自内容密度而非行数约束
