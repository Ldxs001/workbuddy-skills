...
**正向触发**：估算工期/三点估算/PERT/蒙特卡洛模拟/β分布/正态分布/OMP/紧前关系/活动网络图/任务周期估算/P50/P90/生成评估报告

**不触发**：仅询问概念定义 / 纯数学/统计学讨论 / 明确使用其他技能
...
**对话示例：**
```text
用户: "3阶段项目，前端(5/10/20)、后端(8/15/25)、测试(10/20/35)，紧前1→2(FS),2→3(FS)"
→ 推荐β分布+蒙特卡洛+CPM → 用户确认 → 输出HTML报告(甘特图/概率分布/关键路径/分析建议)

"单阶段 O=3 M=6 P=15，直接估算"
"装配式建筑，无OMP值，搜索同类项目参考"
"检查这些任务有没有时间冲突"
"输出评估报告"
```
