# skill-scenario-test 完整使用指南

> **场景测试（Scenario Testing）** — 不以函数为单位，以 **场景链路** 为单位。
> 备份 → 蓝皮书 → 场景+功能测试 → 修复循环 → 回归确认 → 输出报告。
...
3. **不允许修复导致功能失效** — 修复后必须回归确认，与备份前基线对比
4. **修复建议基于场景不越界** — 只建议修复本场景断点，不扩展功能范围

**场景测试 vs 功能测试：**

| | 场景测试 | 功能测试 |
|--|---------|---------|
| 输入 | SKILL.md 声明了什么 | 代码里有什么 |
| 输出 | "历时估算场景：CPM→MC 参数传递断链" | "calc_cpm 语法正确" |
| 修复建议 | "修复参数传递使场景走通" | "增加零值保护" |
| 覆盖 | 用户声称的能力 | 代码里的全部函数 |
...
场景测试结果  | 功能测试结果  | 修复记录  | 回归对比
  S1: 3/3 PASS | D1: 14/14  | 修复2项   | F-0 3→0
