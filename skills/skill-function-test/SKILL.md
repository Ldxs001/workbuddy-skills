...
| `references/s4-noise-testing.md` | S4 脏环境忠实度测试 | 噪音分级 + LLM推理层 + 复盘归因 + 输出格式 |
...
2. **蓝皮书扫描 + 约束提取** — 扫描文件清单 + AST 函数签名 + 引用链路 + **SKILL.md 场景解析 + 约束提取**（阶段A：提取"必须/禁止/铁律/应"等约束关键词，产出结构化约束清单）
3. **询问模式** — 展示场景摘要 + 约束清单 → 用户选择测试范围 + 修复策略（直接修复/询问后修复/仅报告）+ **是否执行 S4 脏环境测试**
4. **场景+功能+S4 脏环境测试** — 先执行 S1-S3 场景测试，再执行 D1-D6 功能测试，最后执行 **S4 脏环境忠实度测试**（阶段B/C/D：LLM推理层 → 噪音方案 → 噪音执行 → 复盘归因）
...
当前版本 **v0.2.7** — skill-standardization 改造：产出物路径迁移+数据目录常量+FAQ质量改进+文档引用修复+版本标准化
