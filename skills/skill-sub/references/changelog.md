...
### 修复
- **标准化审计补全：R-06 H1 位置修复**（紧跟在 frontmatter 闭合 --- 后）
- **标准化审计补全：R-07 触发词质量修复**（移除"直接执行"危险表述，触发词改为具体操作描述）
- **标准化审计补全：C-12 工作流/配置格式修复**（工作流表格→无序列表，配置表 4 列参数表）
- **标准化审计补全：C-11 章节名标准化**（能力边界与限制→约束）
- Skill-standardization 工具 Bug 修复：R-23 路径解析基准目录、fix_section_workflow 覆盖、R-07 auto-fix 前缀死循环（v2.102.4→v2.102.7）
