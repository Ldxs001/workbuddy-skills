...
| **引用校验（citation_check）** | 见「参考文献自动格式化」 |
...
| **引用节（citation_check）** | 固定为 0，完全跳过 LLM 写作，由系统自动生成参考文献列表 |
...
**关联性要求**：引用功能需要同时满足
1. 模板 content 项设了 `citation_check`（含格式如 `[x]=1.`）
...
- 完成后可在界面查看内容、读取、删除
- 文章包含：标题 + 元数据块 + 各节正文 + （可选）事实自检清单 + （可选）引用验证说明
...
**Q：参考文献没有自动生成？**
A：检查三点：① 模板是否设了引用校验（citation_check）；② rag-assistant 是否 ≥ 2.2.10 且在线；③ 该文是否启用了 RAG 并检索到文档。
