...
function addMessage(text, role, id, reasoning) {{
  ...
  if (role === 'assistant' && window.marked) {{
    ...
    // 用户消息/系统消息：先 HTML 转义防 XSS，再渲染公式
  ...
  copyBtn.onclick = function(e) {{
    e.stopPropagation();
    // 复制原始 Markdown 原文，不从 DOM 抠（避免 KaTeX 标注/渲染丢失格式）
    var txt = text;
    // 如果推理已展开，也附上推理内容
  ...
  if (e.key === 'Enter' && !e.shiftKey) {{ e.preventDefault(); sendMessage(); }}
    ...
    def _render_config_tab(self) -> str:
        ...
        <div style="padding:12px 16px;background:#f5f6fa;border-bottom:1px solid #e0e0e0;">
          ...
          <div id="search-config" style="display:{'block' if web_search else 'none'};margin-top:8px;padding:8px 12px;background:#fff;border:1px solid #ddd;border-radius:8px;">
          ...
          function toggleQueryTypes() {{
            var p = document.getElementById('query-type-panel');
...
function hideModal() {{
  document.getElementById('modal-overlay').style.display = 'none';
    ...
    def _handle_chat_history(self):
        """返回当前 session 的聊天历史（按时间戳前缀聚合多行消息）"""
        if not self.agent:
        ...
        if not raw:
            ...
            return
        # 记忆文件格式: "[YYYY-MM-DD HH:MM:SS] role: content\n"
        # content 可能含换行（多段落/表格/代码块），需要按时间戳前缀聚合
        import re
        messages = []
        cur = None
        in_reasoning = False  # 跟踪当前续行是否属于 reasoning
        ...
        for line in raw.split("\n"):
            ...
            if m:
                in_reasoning = False
                role = m.group(1)
                # reasoning 附到前一条 assistant 消息
                if role == "reasoning":
                    if cur and cur["role"] == "assistant":
                        txt = line[m.end():]
                        if txt:
                            cur["reasoning"] = (cur.get("reasoning", "") + "\n" + txt).strip()
                        in_reasoning = True  # 后续续行追加到 reasoning
                    continue
                # 提交上一条
                if cur:
                ...
                # 续行：根据 in_reasoning 决定追加到 reasoning 还是 content
                if cur:
    ...
    # ═══════════════ 查询类型管理 ═══════════════
    BUILTIN_QUERY_TYPES = _BUILTIN_QUERY_TYPES  # 从 agent.py 同步，保持单源
