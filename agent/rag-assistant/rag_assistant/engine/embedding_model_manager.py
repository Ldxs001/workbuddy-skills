...
RECOMMENDED_NLI_MODELS = [
    # 多语言（含中文）— hf-mirror 可拉
    {"id": "MoritzLaurer/mDeBERTa-v3-base-mnli-xnli", "size_mb": 540, "desc": "多语言 NLI（MNLI+XNLI 双源），推荐", "type": "nli"},
    # 纯英文 — hf-mirror 可拉
    {"id": "MoritzLaurer/DeBERTa-v3-base-mnli-fever-anli", "size_mb": 540, "desc": "英文 NLI，MNLI+FEVER+ANLI 三源，SOTA", "type": "nli"},
    # 纯英文 cross-encoder 系列
...
MODEL_DIMENSION_MAP = {
    ...
    "intfloat/multilingual-e5-large-instruc": 1024,
    # 中文/双语系列
...
def get_model_dimension(model_id: str) -> int | None:
    """获取模型的向量维度，未知模型返回 None"""
    dim = MODEL_DIMENSION_MAP.get(model_id)
    if dim is not None:
        return dim
    # 运行时检测
...
def detect_local_embedding_models():
    """
    扫描 data/models/ 目录，发现所有可用的嵌入模型。
    涵盖：系统下载（索引中 type!=rerank/nli）+ 用户手动放置的模型。
    返回: list[dict] — 每个条目含 model_id, path, dimension, size_mb, source
    """
    index = _load_index()
    found = []
    seen_paths = set()

    # 1. 从索引中取已标识的嵌入模型（rerank/nli 跳过不加入 found，但路径标记为已见）
    skip_types = {"rerank", "nli"}
    for model_id, info in index.items():
        path = info.get("path", "")
        if path:
            seen_paths.add(os.path.normpath(path))
        if info.get("type", "").lower() in skip_types:
            continue
        if not path or not os.path.exists(path) or not _is_model_dir(path):
            continue
        dim = get_model_dimension(model_id)
        found.append({
            "model_id": model_id,
            "path": path,
            "dimension": dim,
    ...
    for entry in sorted(os.listdir(MODELS_DIR)):
        ...
        if not os.path.isdir(entry_path):
            continue
        if os.path.normpath(entry_path) in seen_paths:
            continue
        if not _is_model_dir(entry_path):
            continue

        # 通过 modules.json 检测是否为 sentence-transformers 嵌入模型
        # 嵌入模型有 Pooling 模块，rerank/cross-encoder 没有
        modules_file = os.path.join(entry_path, "modules.json")
        is_embedding = False
        if os.path.isfile(modules_file):
            try:
                with open(modules_file, "r", encoding="utf-8") as f:
                    modules = json.load(f)
                # SentenceTransformer 嵌入模型必有 Pooling 模块
                for mod in modules:
                    if "pooling" in mod.get("type", "").lower():
                        is_embedding = True
                        break
            except Exception:
                pass
        ...
        if dim is not None:
            ...
            found.append({
                ...
                "path": entry_path,
                "dimension": dim,
                "size_mb": round(dir_size(entry_path), 1),
                "source": "local",
            })
