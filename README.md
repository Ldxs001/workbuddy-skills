...
| `git-sync` | 将skill代码规范化推送到码云、GitHub并生成ZIP包，自动更新README.md技能列表，附带_meta.json标准化校验、三单一致维护清单机制、敏感信息过滤、SKILL.md规范化审查（R-01~R-10）及渐进式文档体系。新增授权要求章节（41项高风险操作）。 |
...
| `skill-standardization` | Skill 标准化规范引擎 v2.13.4 — 集规范定义、创建器、更新器、改造器于一体，支持 R-01~R-17 审查（含权限分级、敏感信息检测、授权检查、触发条件合规性），新增 permission_checker.py 和 authorization_manager.py，服务于其他 skill 的安全创建/更新/改造。新增 --inject-auth 参数，支持自动注入授权要求章节。 |
