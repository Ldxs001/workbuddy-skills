...
| `skill-standardization` | Skill 标准化规范引擎 v2.21.0。修复：1) parse_simple_yaml_frontmatter() 布尔值解析（true/false → Python bool）；2) 增强 R-13~R-16 自动修正（字段缺失时返回 fix 建议）；3) 修复 R-15 字段缺失检查。 |
