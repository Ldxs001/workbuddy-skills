...
| `git-sync` | 将skill代码规范化推送到码云、GitHub并生成ZIP包，自动更新README.md技能列表，附带_meta.json标准化校验、三单一致维护清单机制、敏感信息过滤和SKILL.md规范化审查。v2.6.8：彻底修复 CredentialHelperSelector 弹出问题（删除无效的 GIT_CREDENTIAL_HELPER 环境变量设置，改为显式配置 credential.helper=store；修复 GitHub 推送失败根源——远程名自动检测替代硬编码 origin；修复 json.load(open(...)) 编码问题 |
...
| `skill-standardization` | Skill 标准化规范引擎 v2.33.0。R-22 数据目录规范检查（含 fix 模式、备份/回滚、dry-run）、refactor 模式增强（加入 R-22 检查步骤）、fix 格式统一。 |
